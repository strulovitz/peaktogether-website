...
### ☀️ SESSION (July 3, 2026 — real-play bugfixes + Parent 22 demon handoff)

Nir play-tested the actual game and it was **far from finished** (tests-green ≠ playable — the classic trap). DeepSeek stopped calling it "polish" and fixed the real bugs Nir hit, one at a time, with Nir as the live judge:

1. **Strafe reversed** — `gameplay.py` had the strafe vector inverted (A went right, D went left). Flipped `str_x/str_z` signs. ✅ Nir confirmed.
2. **Wall-shooting didn't work** — TWO compounding bugs: (a) panel hit-testing (`nav_collision.ray_rect_hit`) used a yaw convention `normal=(cos,sin)` that disagreed with how `render_room` actually draws panels (from the **wall** field). Real rooms put panels on E/W walls with yaw=−π/2, which that convention pointed the wrong way → forward rays came back parallel → no hit. Added `ray_rect_hit_wall(ray, wall, center, w, h)` (wall-basis, matches renderer) and switched `nearest_panel` to it. (b) `resolve_shot` stores the **pair-id** in `state.lit`, but `render_room.panel_is_on` only checked **asset-ids** → panels never turned colored even on a hit. Made `panel_is_on` also return True if `pair_id in lit`. ✅ Nir confirmed shooting works.
3. **Drawings tiny & looked stretched** — drawings were aspect-correct but sized by `room_px_per_m=600` while text used `text_px_per_m=160`, so text was ~3.75× bigger and drawings looked small. Per Nir: make drawings **as tall as the text, proportions preserved**. Changed `build/room_maker.py` to size the drawing from a target height = the text panel height, width from the figure's true aspect, capped at `panel_max_w/h`. Regenerated all 20 room runtimes (Stage-6-only regen from the baked manifest — no image re-baking; all 20 converged, no RoomTooDense). ✅ Nir confirmed better. Regen helper (temp, not committed): `%TEMP%\opencode\regen_room_runtimes.py`.

All **455 green** throughout. Commits `f753264` (fixes + regen runtimes) and the room_maker source. **NOTE: `quake/build/` matches root `.gitignore` `build/`** but the files are already tracked — use `git add -f` to commit build-source changes.

**🩸👹 PARENT 22 LAUNCHED (handoff written): BUILD THE DEMON.** Nobody ever rendered the demon (game logic exists in `gameplay.py`: spawn/hit/kill events + HP; nothing drew it). Nir's exact spec (in the handoff verbatim):
- Demon from **MANY circles** (NOT the old 4-shape minimalism): **~100 pink** circles = body; **~10 white** = mouth (5 upper + 5 lower teeth); **2 bigger blue** = eyes, each with a **smaller black pupil** inside, each pupil with a **tiny white glint** inside.
- Shot **3 times → whole demon explodes** (all circles fly random directions, then disappear). (Data note: `enemy.health` is currently 5 → set to 3.)
- **Alcove revealed when the demon appears** — the final proof panel is a hidden door; opening it reveals the little room the demon was locked in. (`render_room._build_alcove` exists but is drawn unconditionally today — gate it on demon-spawn.)
- On **kill → blood-red equations on the ceiling** above each station (partly exists: ceiling eqs already tint red when `room_id in state.cleared`).
...
**Part 3 — FULLY SHIPPED ✅ (July 5, 2026, ~2 AM Israel):**
- Nir did the itch.io page (description + cover `colored-walls` + 5 screenshots + tags + Public) and the FileZilla → Dreamhost upload (home `index.html`, `arcade/index.html`, `arcade/quake/index.html`, and all 6 `images/quake-principia-*` files). Home page Quake block flipped to **▶ PLAYABLE / "Quake: Principia"**; arcade "then/now" photo swapped to `quake-principia-demon.png`.
- **🎉 ALL THREE PARTS DONE. Quake: Principia is LIVE:** playable game (keyboard/mouse + T.16000M joystick + Xbox controller), one-command Windows exe+zip, on **itch.io (primary, Public)** + **GitHub Releases (mirror)**, with a full **peaktogether.me/arcade/quake/** page (gameplay video via jsDelivr + screenshot gallery + controls) and a Playable home-page feature.
- **Optional future polish (not blocking, Nir may want later):** add a Quake "Play Free" button to the site header menu (like Descent); optional YouTube trailer link on itch; the earlier open in-game items (4 Asymptote figures on placeholders — actually all 20 compile now; equation-panel sizing; controls hint) remain minor/optional.

## 12. ON RESTART / AGENTS.md
🌙 **ON RESTART:** Read this WORKFLOW.md first, then the **Commentaries**, then ask Nir what's next.
🌙 **CURRENT STATE (July 5, 2026, ~2 AM — GAME SHIPPED! 🎉):** **Quake: Principia is FINISHED and LIVE.** Three finishing parts all done this session: (1) **Controller support** (Parent 23, pyglet-native) — boyfriend keyboard OR T.16000M joystick (move/turn + throttle-to-run), girlfriend mouse OR Xbox (look/aim + fire), additive; (2) **Packaging** — one-command PyInstaller Windows exe + zip + sha256 (pt_runtime bootstrap), shipped to **itch.io (primary, Public)** + **GitHub Releases (mirror, tag `quake-principia-v1.0.0`)**; (3) **Website** — `peaktogether.me/arcade/quake/` upgraded to Playable (gameplay video via jsDelivr + 5-screenshot gallery + real controls), home page + arcade landing updated, all uploaded to Dreamhost. **468/468 green. Everything committed + pushed.** See the July 4–5 session block above for full detail. Optional future polish: header "Play Free" menu button for Quake, YouTube trailer on itch, minor in-game equation-panel sizing / controls hint.

**Older state (July 3, 2026 evening):** All 4 child rooms rewritten + rebuilt. Demon bugs FIXED (dead demon on re-entry, alcove never visible). Alcove draws through wall with depth disabled. Ceiling equations sized to actual PNG aspect ratio (capped 3m wide). Non-geometry panel text enlarged (28pt Asymptote font). Station numbers (gold, 1,2,3...) in upper-left of each text panel. Law_2 text panels FIXED. **468/468 green.**
...
**All 16 child rooms are fine** (lemma_3,4,5,6,7,9,10,11,12 + prop_1,2,6,7,11,13,15). **The 4 DeepSeek-written rooms need FULL REWRITES from scratch:** law_1, law_2, prop_4, lemma_2 — each gets a child writing a COMPLETE new .room file (stations, panels, text, colors, ceilings). Prompts: `CHILD_PROMPT_{LAW_1,LAW_2,PROP_4,LEMMA_2}_NEW.md`.
