...
from camera import (forward_from_angles, look_at,
                    perspective as _perspective_matrix, FOV_Y_DEG, NEAR_M, FAR_M)
...
# FOV_Y_DEG / NEAR_M / FAR_M and the perspective matrix now come from camera.py
# (imported above as _perspective_matrix) so the viewer and the game share ONE
# source of truth and cannot drift. (Parent 11 reconciliation.)
...
def run(floorplan_path: Path) -> int:
    ...
    def on_draw():  # noqa: ANN001
        ...
        proj = _perspective_matrix(FOV_Y_DEG, aspect, NEAR_M, FAR_M)
        ...
        # Parent 11 changed draw_graph's signature to (view, proj, aspect, fp, state);
        # DeepSeek updated this call site to match (Parent 11 missed it).
        draw_graph(view, proj, aspect, fp, state)
