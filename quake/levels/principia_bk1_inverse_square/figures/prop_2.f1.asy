...
pair D = (3.0,0.0);
pair _u_E = (6.0,0.0);
path _u_path = A--B--C--D--E;
path SA = A--S;
path SB = B--S;
path SC = C--S;
path SD = D--S;
path _u_SE = E--S;
path triSAB = S--A--B--cycle;
path triSBC = S--B--C--cycle;
path triSCD = S--C--D--cycle;
pair _u_S = (0.0,4.0);
pair A = (-6.0,0.0);
pair B = (-3.0,0.0);
pair C = (0.0,0.0);
...
pair _u_E = (6.0,0.0);
path _u_path = A--B--C--D--E;
path fB = B--(shift(10*unit(S-B))*B);
path fC = C--(shift(10*unit(S-C))*C);
path fD = D--(shift(10*unit(S-D))*D);
path SA = A--S;
path _u_SE = E--S;
