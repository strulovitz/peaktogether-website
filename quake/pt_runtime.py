"""pt_runtime.py — Peak Together frozen-exe bootstrap for QUAKE.

Adapted from Descent QED's pt_runtime.py (proven in the shipped Descent build).
Call bootstrap("Quake") at the very top of app.py, before loading any assets:
  - When frozen by PyInstaller (one-folder), bundled data lives in sys._MEIPASS
    (the "_internal" folder), NOT next to the .exe. We point the working dir there
    so relative / __file__-based asset loads resolve.
  - Saves, logs and the crash log go to %LOCALAPPDATA%\\PeakTogether\\Quake so the
...
def bootstrap(game_slug: str = "Quake", game_title: str = "Quake") -> None:
    ...
    if getattr(sys, "frozen", False):
        ...
        # "_internal" folder). Point the asset base there so relative /
        # __file__-based asset loads resolve. Fall back to the exe folder.
