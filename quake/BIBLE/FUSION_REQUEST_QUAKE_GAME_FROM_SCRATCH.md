...
**⚠️ A HARD PERSONAL CONSTRAINT — design around this, do not work around me.** Here is something essential about me that must shape your whole answer to this section. **I do not know how to code at all** — I rely 100% on AI for all the vibe-coding, the design, and everything. And **in exactly the same way, I do not know any mathematics at all** — and I rely 100% on AI (the frontier models available in **OpenRouter** — i.e. the kind of models you yourselves are) to do this geometry conversion for me. I do **not** rely on my local coding agent (DeepSeek) to *understand* the math, and I absolutely do **not** rely on my own thinking. So whatever pipeline you recommend below **must not require me to understand the proofs, to read the mathematics, or to manually draw / reconstruct / trace any figure myself in any software.** I do not understand these proofs; I could not even if I wanted to; and I do not want to. Do **not** answer with "the best approach is for you to think about each illustration and recreate it by hand in [some tool]" — that is impossible for me and would defeat the entire project. **The process must be as mechanized and automated as possible, and entirely AI-driven.** It may chain several different AIs together (e.g. one model to read/understand the scanned figure, another to emit the figure source, another to verify it) — but the *intelligence* must always come from AI, never from me. **What I personally can do is only mechanical:** copy-paste between chats, ask my coding agent to help write a Python script, fetch and hand over the PNG scans (e.g. from Archive.org), and install software/libraries if needed. That is the full extent of my involvement. Design the pipeline so that everything requiring *understanding* (of the geometry, the math, the figure) is done by AI, and everything left for me is pure mechanical clicking, pasting, fetching, and running scripts.
