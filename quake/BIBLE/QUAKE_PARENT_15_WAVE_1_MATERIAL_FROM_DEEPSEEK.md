...
**DeepSeek ruling:** **DECISION FOR YOU.** Two honest options: (a) **drawn-fresh FIGURE room** — we draw the ellipse + one pair of conjugate diameters + the circumscribed parallelogram ourselves (1 station, statement-as-figure, since the page has no diagram and no proof steps); or (b) treat as a one-station **equation/statement-as-figure** with a minimal sketch. Either way it is **one station, no multi-step proof** (there is no proof). Flag this honestly to Nir in your deliverable.
...
**[DeepSeek summary of Wave 1]:** Of Parent 7's 9 "figure-less" rooms, Wave 1 covers 8 — and **all 8 should have a colored thing to shoot**: `lemma_3/9/10` + `prop_2` are real diagrams; `prop_4` + `law_2` are equation-as-figure; `law_1` is a multi-station foundation room; `lemma_12` is a one-station drawn-fresh statement (no Newton proof). The 9th, `prop_15` (Kepler's 3rd, T² ∝ a³), is Wave 2 and is also equation-as-figure.
