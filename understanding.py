...
PHYSICAL MODEL (corrected):
  * Each sign is real glass. Its transparency is a FIXED, baked property -- it does
    NOT change as you approach (no observer effect). We draw signs at full alpha and
    let the PNG's own alpha be the only transparency.
  * What changes with distance is the AIR: FOG (color = FOG_COLOR) sits between signs.
    Farther signs are seen through MORE fog (dimmer/washed) and are OUT OF FOCUS
...
Input (UNCHANGED): wheel = depth(=zoom), mouse + right stick = pan, CTRL = engineer
unlock, ESC / back-out = exit."""
...
SIZE_FALLOFF = 0.85   # perspective decay rate
...
FOG_COLOR    = (0.04, 0.05, 0.07)  # BLACK fog (matches dark bg). Flip to ~(0.9,0.92,0.95) for WHITE fog.
FOG_PER_D    = 0.55   # fog opacity added per unit distance (0..1, clamped)
FOG_MAX      = 0.92   # never fully opaque, so deep structure still ghosts through
...
class UnderstandingMode:
    ...
    def _load_panel_ladder(self, layer):
        ...
        for r in range(1, BLUR_RUNGS):
            rungs.append(self._surface_to_texture(render.blur_surface(surf, r * BLUR_STEP)))
        return rungs
    ...
    # ---- continuous appearance (NO opacity change; size + focus + fog) -------
    def _size_fill(self, d, win_w):
    ...
    def draw(self, cache, win_size):
        ...
        if foc_rungs:
            fill0 = self._size_fill(abs(self.focus - focused_i), w)
        ...
        for i in order:
            ...
            if rungs:
                ...
                render.render_rich(cache, title, px + 8, py + draw_h + 6,
                                   ...
                                   fontsize=max(10, int(BASE_FONTSIZE * min(1.0, fill / CLOSEUP_FILL))))
            else:
                fs = max(10, int(BASE_FONTSIZE * min(1.0, self._size_fill(d, w) / CLOSEUP_FILL)))
    ...
    def _draw_minimap(self, rungs, draw_w, draw_h, w, h):
        # thumbnail = blurriest rung (cheap, already loaded); aspect from sharp rung
        _, sw, sh = rungs[0]
        mm_w = MINIMAP_W
        mm_h = mm_w * (sh / sw)
        ...
        # view rectangle: our screen window mapped into the full sign.
        # focused sign center is at screen-center + pan; full sign top-left in screen:
        sign_left = (w - draw_w) * 0.5 + self.pan_x
        sign_top  = (h - draw_h) * 0.5 + self.pan_y
        # fraction of the sign currently visible (the screen window):
        vx = (-sign_left) / draw_w
        vy = (-sign_top) / draw_h
        vw = w / draw_w
        vh = h / draw_h
        rx = mx + max(0.0, vx) * mm_w
        ry = my + max(0.0, vy) * mm_h
        rw = min(1.0, vw) * mm_w
        rh = min(1.0, vh) * mm_h
