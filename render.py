...
import io
import math
import re                         # Brief #10: value-arc regex
import numpy as np
...
class TexCache:
    ...
    def get_rich(self, text, color=(0.95, 0.96, 0.98), fontsize=15, blur=0.0):
        """Brief #10: cache mixed prose+math panels with optional blur."""
        key = (text, fontsize, color, round(blur, 1))
        if key not in self.cache:
            self._prune()
            surf = rich_to_surface(text, color, fontsize)
            if blur > 0:
