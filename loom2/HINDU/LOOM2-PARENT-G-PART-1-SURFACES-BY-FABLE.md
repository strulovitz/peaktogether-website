# LOOM2 — PARENT G, PART 1 of 3: core/surfaces.py — BY FABLE

> Saved VERBATIM, word-for-word, as delivered by Claude Fable "Parent G", 2026-07-08.
> This is the first of Parent G's three modules (surfaces.py -> scene.py -> main.py).
> The runnable code is extracted to `loom2/core/surfaces.py`.

---

Thank you Nir!!! 💖 Decisions locked: resizable=False, strict validation, boot sanity print, long thoughtful files. Building now — here is module 1 of 3: core/surfaces.py, with the whole answer devoted to it as required.
...
Honest delivery notes 🧿
    ...
    What I verified: every formula body traced character-against-comment; all 15 self-test values computed by hand (e.g. 0.16(22)=0.64, 0.03⋅102⋅sin90°=3.0); the ridge broadcasting trap reasoned through for all four scalar/array mixes.
    What I did NOT do: execute this code. I cannot run anything from this chat. The self-test block exists precisely so DeepSeek/you can run python -m core.surfaces from the repo root and get a real verdict.
    Decisions made (all visible above, none hidden): K_CANNON = 0.03 with design domain v∈[0,10], θ∈[0°,90°] → peak z=+3.0 (my approved recommendation); the + 0.0 * y shape-keeper in ridge (mechanical, but reported); KeyError for get(); math/config imports omitted as unused; additive __main__ self-test.
    No # CONTRACT-ISSUE: flags — the contract was clean.
    Size: ~310 lines by my count (an estimate — I can't run wc). Over the ~120 estimate, per your "long and full of thought" ruling; the surplus is documentation and the self-test, not padding.

Next answer: core/scene.py. Say the word! 😊🏔️🎼
