# LOOM2 — PARENT G, PART 2 of 3: core/scene.py — BY FABLE

> Saved VERBATIM, word-for-word, as delivered by Claude Fable "Parent G", 2026-07-08.
> This is the second of Parent G's three modules (surfaces.py -> scene.py -> main.py).
> The runnable code is extracted to `loom2/core/scene.py`.

---

Thank you Nir!!! 💖 (And the message arrived perfectly — thank you for the credits!) Here is module 2 of 3: core/scene.py, whole answer devoted to it.
...
Honest delivery notes 🧿
    ...
    What I verified: every check traced by hand against the verbatim test_saddle/scene.json — it passes every validator above (all 13 keys, saddle in REGISTRY, 3+1 lines, 4 options with only C correct, camera_limits complete, totem inside domain). The frozen docstrings are preserved verbatim, extended below.
    What I did NOT do: execute anything — I can't run code from here. The self-test (python -m core.scene from repo root) gives the real verdict against the live files.
    Decisions made (all visible, none hidden): unknown keys rejected at all three levels (typo protection, the strict spirit you chose); G2.5-A extras tolerated + lightly type-checked, not stored; explain and success_text may be empty (test_saddle's correct option ships explain: "" — required non-empty would reject the live scene!); SceneError(ValueError) additive class; UTF-8 explicit (emoji!); 0-byte file check; _MAX_GRID_VERTS freeze guard; totem bounds inclusive; bool-is-not-a-number guard.
    No # CONTRACT-ISSUE: flags — one near-miss worth recording: the contract says "camera_limits keys present," which G3.2-A + Q5 softened to "filled with defaults"; the amendment wins, as documented in the docstring.
    Size: ~340 lines by my count (estimate). Over the ~130, per your long-and-thoughtful ruling — the surplus is error quality and documentation.

Next and final answer: main.py — the heartbeat. Say the word! 😊🎼🚀
