...
- **📐 TOTEM CANON (locked):** A7 signature LIVE = `TotemVisual.draw(self, view_proj, totem_state,
  ...
  🚫 FLAT-SHADING VIOLATION (being fixed): the delivered helix is flat-colored (no per-vertex
  lighting) — BREAKS Nir's IRON RULE (no flat shading, ever; everything Gouraud). Fable D asked
  to redeliver a GOURAUD helix (courier note `COURIER-TO-PARENT-D-GOURAUD-HELIX.md`); NOT
  acceptable as shipped — fix before Parent E.
