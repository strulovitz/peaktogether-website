# LOOM2 — PARENT F DELIVERY (graphics/hud.py + core/input_map.py) — BY FABLE

> Saved VERBATIM, word-for-word, as delivered by Claude Fable "Parent F", 2026-07-08.
> Chunk = Gita G3.7 (hud.py) + G4.4 (input_map.py), built against the amended contracts
> (SUTRAS-2-A, Gita G3.7-A) — Homeworld-style moderngl 2D overlay, NOT pyglet.
> The runnable code is also extracted to loom2/graphics/hud.py and loom2/core/input_map.py.

---

✅ Round 2 absorbed — and message received, loud and clear: no more menus, decide beautifully, BUILD. Here is the code — complete, both modules, one delivery. 🛠️

Design decisions I made (per  Nir's "most beautiful and professional" ruling)

    Constructor: Hud(window, renderer) — context from renderer.ctx (fallback moderngl.get_context() if renderer is None). Parent G: boot step 6 is Hud(window, renderer).
    Quiz bar layout: question top-left; beneath it a small gray encouragement line ("🎧 Listen to all four — as many times as you like." — the UPANISHADS §3 requirement that the UI says so); A–D buttons left, OK + 💡HINT center; right third = feedback area (green hint lines above pink explanation, wrapped).
    🔊 sits at the right edge of the playing button; selected button = brighter fill + white frame; OK label dims when nothing is selected.
    SLICE mode: quiz bar shows one green helper line ("🔪 Glass Blade — WASD move · ◀▶ rotate · ▲▼ tilt · Enter play · C exit") instead of buttons.
    SCENE_TRANSITION celebration: success_text in warm yellow, wrapped, centered over the graphics; "✅ Correct!" in green in the feedback area.
    Win: "YOU WIN!!!" 72 px light-blue, blinking ~1 s (frame-counter — time isn't in my allowed imports; comment explains); final scene's success_text (the closing line) steady beneath it, non-blinking.
    Blink / drag constants at the tops of the files with comments, as instructed.
    Atlas: one 1024² RGBA texture, shelf packer, glyphs baked at 64 px (white fill + baked black stroke, tinted at draw time), color emoji cells from seguiemj.ttf drawn untinted; lazy baking so any emoji Nir writes in scene JSON just works.
    SIZE NOTE (G1.1 rule 4): hud.py ≈ 430 lines — the atlas is the irreducible cost of the no-pyglet ruling. Reported, not hidden. input_map.py ≈ 150 lines.
...
Hand-off notes for DeepSeek / Parent G:

    Boot step 6 is Hud(window, renderer) (blessed additive signature); Hud never touches the window object — it's accepted for contract shape only.
    Hud is scene-less safe: constructible and drawable before the first set_scene (frame-1 scene_changed flow works as described in A3).
    Hud sets its own 2D GL state in _flush() every frame; it must be called after renderer.composite() (frozen frame step 7). It never binds framebuffers — it draws to whatever is bound (ctx.screen).
    input_map emits CONFIRM for Enter (never SLICE_PLAY — _route_slice accepts both; the enum member stays for the future gamepad mapping).
    Both W/S and a live mouse drag may emit TOTEM_Y in the same poll; game_state's zero-each-frame accumulation handles it (last/summed intent — solo mode still works).
    Tunables at file tops: WIN_BLINK_FRAMES = 30 (frame-based — no time import allowed), DRAG_FULL_PX = 160.0.
    Test with data/scenes/test_saddle/ — emoji title lines exercise the lazy atlas path deliberately.

Checklist — complete:

✅ All scriptures  ✅ Q&A rounds 1–2  ✅ Decisions made (no menu, per Nir)
✅ CODE: graphics/hud.py + core/input_map.py — one delivery

It was an honor to carry G3.7 and G4.4. May the quiz bar be kind, and may the players hear the mathematics. 🧿🎧❤️
