...
homeworld/console.py — Bridge class (B3, FLEET ZONE, APOCRYPHA 3.4): live fleet matrix A (ships as columns, rows K/B/M/S/J/U) + RANK/RESOURCES/SELECTED readouts + selected-column highlight following the Pilot's TAB; read-only MatrixGrid; Bridge(overlay).on_pulse/on_frame — SUPERSEDED by corrected B3 below
homeworld/console.py — CORRECTED B3 Bridge (always space): FORMATION P (live 3×n positions, rows e1/e2/e3) + ORDER (shared c1/c2/c3 sliders + fuel line) + TRANSFORM M (editable 3×3, det/rank readouts, collapse warning, APPLY/RESET/SCOPE); on_pulse(pointer, snap, ui-dict)/on_frame; callbacks on_coeff/on_transform — WORKING
...
B3 shipped: console.py — Bridge class (the FLEET ZONE, APOCRYPHA 3.4 zone 1): live fleet matrix A with SHIPS AS COLUMNS (rows K,B,M,S,J,U), RANK/RESOURCES/SELECTED readouts, selected ship's column highlighted (follows the Pilot's TAB). Wiring contract: Bridge(forge.overlay2d), .on_pulse(pointer, snap, selected_id) once per PULSE, .on_frame(w, h) once per FRAME. Reads snapshots only, never mutates fleet. app.py re-emitted whole with 6 additive wiring changes (docstring, import Bridge, self.bridge=Bridge(overlay2d), tick pause-restructure + on_pulse, on_frame block, one print) — all owner feel-numbers untouched (verified by git diff). Game is now TWO-PLAYER. Run `python app.py`.
B3 CORRECTED (always space — supersedes the capability-matrix console above): Nir rejected the K/B/M/S/J/U signature spreadsheet; the console is now Part 2 §2.1/M1/M8/M10. console.py Bridge has THREE zones: (1) FORMATION P — commanded squad's live positions as a 3×n matrix, rows e1/e2/e3 colored like the basis arrows, updates every pulse as ships fly; (2) ORDER — the c1/c2/c3 coefficient sliders wired to the SAME shared coefficients the Pilot's keys edit (Navigator drags → the Pilot's ghost construction moves in space), plus the fuel line (staged legs vs diagonal = triangle inequality); (3) TRANSFORM M — an editable 3×3 (starts identity) ghost-previewed in space as p→M·p, with det(M)/rank(M) readouts and a "COLLAPSE TO A PLANE/LINE" warning, APPLY/RESET/SCOPE buttons. New order ApplyTransform(squad, matrix) (squad 0 = whole fleet) in orders.py; sim `_ingest` branch sets each member's plan to [M @ pos] (reuses cruise movement) and emits TRANSFORM_APPLIED. New referee.determinant(M) = signed det (orientation/reflection). app.py: Bridge gets on_coeff/on_transform callbacks; shared self.coeffs; dashed p→Mp ghost pool + amber real_eigen_axis line. The signature matrix leaves the console (belongs to combat M4 / economy M3). Run `python app.py`.
