# COMMENTARIES — repository memory. Updated: July 4, 2026 (forge + bloom)
...
homeworld/settings.json — human-editable config (title, version, size, vsync, bloom_strength, exposure, seed) — WORKING
homeworld/forge/__init__.py — forge package exports (Forge, PULSE_DT, Camera, VObjects) — WORKING
homeworld/forge/app.py — Forge class: window, GL context, 10 Hz accumulator loop, scene FBO -> bloom -> screen — WORKING
homeworld/forge/camera.py — Camera: ORBIT mode + look_at/perspective math (NT 1.3) — WORKING
homeworld/forge/shaders.py — GLSL shaders: line ribbon + bloom pipeline (fullscreen tri, blit, blur, composite) (NT 1.5/1.6) — WORKING
homeworld/forge/bloom.py — Bloom: 3-FBO classic bloom (RGBA16F scene, downsample, separable Gaussian, composite + tone map) (NT 1.6) — WORKING
...
Not yet implemented (later packages): text/Label, SpannedBox, Ellipsoid,
...
run.bat). Now includes bloom (halos, hot cores, flaring crossings).
