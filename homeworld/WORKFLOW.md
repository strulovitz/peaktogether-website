...
### July 5, 2026 — CORRECTED B3 DELIVERED & INTEGRATED ✅ (always space — the game Nir designed)
Fable owned the miss ("I fucked up the game… B3 misread the founding design; console = Part 2 §2.1/M1/M8/M10, 'always space'; the signature matrix leaves the console for combat M4/economy M3"). Answered his Batch-3 (orders.py, sim _ingest pattern, referee def lines, fuel/context one-liners). He delivered 5 pieces; all applied:
1. **referee.determinant(M)** appended (signed det = volume factor + orientation/reflection).
2. **orders.py** re-emitted whole — new `ApplyTransform(squad, matrix)` (squad 0 = whole fleet; 3×3 row-major tuple).
3. **sim.py** — `ApplyTransform` added to the orders import + a new `_ingest` branch: sets each member's plan to `[M @ pos]` (reuses the cruise movement) and emits `TRANSFORM_APPLIED`.
4. **console.py** full rewrite — three zones: **FORMATION P** (commanded squad's live positions as a 3×n matrix, rows e1/e2/e3 colored like the basis arrows, updates every pulse); **ORDER** (c1/c2/c3 sliders wired to the SAME shared coeffs the Pilot's keys edit — Navigator drags → Pilot's ghost moves; + fuel line staged-legs vs diagonal); **TRANSFORM M** (editable 3×3, det/rank readouts, COLLAPSE warning, APPLY/RESET/SCOPE).
5. **app.py** re-emitted whole — Bridge gets `on_coeff`/`on_transform` callbacks + shared `self.coeffs`; dashed `p→Mp` ghost pool + amber `real_eigen_axis` fixed-axis line; TRANSFORM_APPLIED print.
- **Confirmed** `real_eigen_axis` returns a unit 3-vector (Fable's call site handles it). All flat (imports already flat).
- **Verified:** py_compile OK (referee/orders/sim/console/app); imports resolve; `determinant(2I)=8.0`, `determinant(diag(1,1,-1))=-1.0`, rotation axis `[0,1,0]`; **headless sim test: fighter (6,0,0) + vertical-rotation ApplyTransform → (0,0,-6)**, TRANSFORM_APPLIED fired; `python fleet_demo.py` → **12/12 GREEN**.
- Saved verbatim to `BIBLE/FABLE-DELIVERABLE-13-corrected-B3-formation-transform-(always-space).md`. Committed with Fable's exact message + pushed. COMMENTARIES updated (old capability-console note marked SUPERSEDED).
- ⏳ **Nir to run `python app.py`** and REPORT BACK: (1) clean run? (2) is THIS the instant matrix↔space link he meant? (3) what's wrong — sizes/speeds/feel. Try: TRANSFORM rotation `0 0 1 / 0 1 0 / -1 0 0` (det +1, amber axis), `2·I` (det +8), zero a row (COLLAPSE TO A PLANE), `-1` on a diagonal (mirror, det −1).

### 🌳 July 5, 2026 — TRUNK / BRANCH ARCHITECTURE (Fable becomes the grandparent)
**New working model (Nir):** Parent 2 (Fable) becomes the **TRUNK / grandparent**. Instead of one fragile long-lived parent, Fable now writes **sealed, self-sufficient documents**: a shared **BRANCH CHARTER** + one **mission brief per mini-game**. Later, each **branch-parent** = a fresh Fable chat that inherits {founding docs + `charter.md` + the current `app.py` + ONE brief} and builds that ONE mini-game. This survives the trunk's context death.
- **M = MECHANIC** (Mechanics Canon §5.2), NOT "matrix". The 16 missions are the campaign list (Book VI). Each mini-game = one MECHANIC wrapped in a mission's fiction.
- **DeepSeek's standing job for trunk answers:** each WHOLE answer Nir pastes → (1) archive it VERBATIM in its own clearly-named file `FABLE-TRUNK-ANSWER-NN-...md`, AND (2) split it EXACTLY as Fable delineates ("FILE: name — ...") into individual files, all in `homeworld/BIBLE/`, hyphen/underscore names (no spaces), commit + push each time.
...
### 🌐 SESSION (July 5, 2026 — WEBSITE PAGE for Homeworld, like Descent/Quake)
Nir made a **combined gameplay video** (4 clips: carousel-spin → collapse → mirror → tumble-upright, joined losslessly with ffmpeg concat → `C:\Users\nir_s\Videos\Captures\homeworld-combined.mp4`, 42s, 1280×720@60, 15.9 MB — **do NOT change/re-encode it, Nir's instruction**) + 4 screenshots. Built the website presence exactly like Descent/Quake:
1. **Game page `arcade/homeworld/index.html`** — cloned Quake's structure/style: **🔨 Building — Sneak Peek** badge (`game-badge building`, orange), **🌌** space-emoji title, gameplay video (jsDelivr from the repo) + poster, hero = `homeworld-spin-plus-grow-swirl.png`, real-screenshot gallery (4), the "then/now" (Homeworld 1999 → ours), the twist (fleet = matrix, transform reshapes it), the mountain (Strang linear algebra, "a good basis", SVD finale), Pilot(keyboard)/Navigator(mouse) roles, **accurate controls** (keyboard+mouse only — NO controllers claimed; Homeworld's joystick/gamepad are still stubs), sneak-peek download (itch + GitHub prerelease), run-from-source, lightbox script.
2. **`arcade/index.html`** — graduated Homeworld from a "coming soon" teaser to a **🔨 Building now** entry (real 4-screenshot gallery + description + link to its page); removed the old placeholder teaser; `id="homeworld"` anchor moved to the new entry (top cover-thumb still links to it).
3. **Homepage `index.html`** — added a Homeworld `featured-game` after Quake: **🔨 BUILDING** badge, **🌌** title, description, button "🌌 See the Sneak Peek →" → `/arcade/homeworld/`.
4. **Assets copied into repo:** `images/homeworld-{spin-plus-grow-swirl,battle-line,expand-scatter-2-2-2,sheer}.png` + `images/homeworld-combined-poster.jpg` (ffmpeg frame) + `arcade/homeworld/homeworld-combined.mp4` (unchanged copy). Reused existing `homeworld-{2,3,4,6}.jpg` (original-1999 art) + `homeworld-cover.jpg`.
- **Video served via jsDelivr** (like Quake): `https://cdn.jsdelivr.net/gh/strulovitz/peaktogether-website@master/arcade/homeworld/homeworld-combined.mp4` — NOT Dreamhost. No CSS change (reused `.gp-*` + `.doom-*` + `.game-badge.building`) → no style.css cache bump.
- **NEXT (Nir's manual steps, like Descent/Quake):** FileZilla → Dreamhost upload `index.html`, `arcade/index.html`, `arcade/homeworld/index.html`, and the 5 new `images/homeworld-*` files (NOT the mp4 — jsDelivr serves it from git). Optionally add itch.io cover+screenshots and set the itch page Public.
- **Late tweaks (Nir, same evening, all pushed):** then/now "Now" image → `homeworld-spin-plus-grow-swirl.png`; then/now "Then" image → `homeworld-3-wide.png` (from Downloads); top hero image → `homeworld-a-good-basis-hero-graphics.png` (Nir generated it from `homeworld/PROMPT-HERO-GRAPHICS.txt` — Strang's Four Fundamental Subspaces); arcade Homeworld mini-gallery → original Homeworld 1999 photos (`homeworld-6/4/3/2.jpg`). Also wrote the Descent-style **hero-graphics prompt** + gave Nir full **itch.io** step-by-step (cover/screenshots/description/tags/Public).
- 🌙 **END OF DAY July 5, 2026 — everything committed + pushed. TOMORROW = 🧵 LOOM** (new game — Loom/The Dig, sound/sonification puzzles; Nir's Sonification project). On restart, ask Nir how he wants to start LOOM (brainstorm / new `loom/` folder / which material).
