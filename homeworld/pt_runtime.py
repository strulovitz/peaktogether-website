"""pt_runtime.py - Peak Together frozen-exe bootstrap for HOMEWORLD.

Adapted from the shipped Descent QED / Quake: Principia pt_runtime.py.
Call bootstrap(...) at the very top of app.py main(), before loading any
assets (settings.json, the content/ folder):
  - When frozen by PyInstaller (one-folder), bundled data lives in sys._MEIPASS,
    NOT necessarily next to where the user launched from. We chdir there so the
    game's CWD-relative loads ( open("settings.json"), ContentDB("content") )
    resolve.
  - Saves, logs and the crash log go to %LOCALAPPDATA%\\PeakTogether\\Homeworld so
    the game works even if installed in a read-only location.
  - Uncaught exceptions are written to a crash log and shown in a message box.
This module keeps Homeworld FLAT (a plain sibling in homeworld/, no packages).
"""
...
_GAME_SLUG = "Homeworld"
_GAME_TITLE = "Homeworld: A Good Basis"
_APPDATA_DIR: Path | None = None
_BASE_DIR: Path | None = None


def bootstrap(game_slug: str = "Homeworld",
              game_title: str = "Homeworld: A Good Basis") -> None:
    """Call once at the very start of app.py main(), before loading assets."""
    ...
    if getattr(sys, "frozen", False):
        ...
        # asset base there so CWD-relative / __file__-based loads resolve.
        # Fall back to the exe folder.
