12/12 — the referee has spoken! ❤️ The mathematical heart is beating. (Small tip for the future: always run from the root homeworld folder — e.g. python -m fleet.demo — rather than from inside a module folder. It worked this time, but the -m form from the root is the supported way, and today's package requires the root because it wires all modules together.)

Now the milestone package: THE WIRING (New Testament build step 9) — the root app.py that connects forge + helm + fleet into one living game shell. From today, run.bat launches the actual game: a mothership, three fighters, and your keyboard commanding a real combination order c1​e1​+c2​e2​+c3​e3​ — the core mechanic of Mission 1, playable tonight.

Below are 3 complete files. Send each to DeepSeek exactly as-is.

File 1 — app.py (at the repository root, next to run.bat)
...
File 2 — run.bat (updated — now launches the real game shell)
...
File 3 — settings.json (version bump)
...
📋 YOUR STEPS
    ...
    Send to DeepSeek: the 3 files, verbatim, commit message: NT step 9: app.py wiring — forge+helm+fleet, shakedown scenario with combination orders — and update COMMENTARIES.md.
    Run: double-click run.bat from the root folder.

👀 WHAT YOU SHOULD SEE — AND DO (this is the first playable build!)

The window opens straight into space (First-Five-Minutes Doctrine!): the glowing grid, three red/green/blue engine arrows labeled e1, e2, e3, a golden mothership, and three cyan fighter darts. Console prints the controls. Then try this exact flight, in order:

    Hold W for about a second. A white ghost arrow grows out of the fighters' formation, with a blue dashed leg and a floating label like (+0.0, +0.0, +2.0). Nothing moves yet — you are composing a combination.
    Hold D for a second. A red dashed leg joins in; the ghost arrow now points diagonally: you're looking at c1​e1​+c3​e3​ drawn live.
    Press ENTER. The ghost vanishes, the console prints your order (e.g. ORDER: squad 1 <- 2*e1 + 2*e3 (diagonal)) — and all three fighters fly the diagonal together, noses turning into the flight direction, comet trails behind them.
    Press X, compose again, press ENTER. Now they fly component-by-component: first along e1​, then along e3​ — two sides of the parallelogram instead of the diagonal. Same destination, different path. That's Bible mechanic 2.1, alive.
    TAB cycles the white highlight between ships (mothership too); C recenters the camera on the highlighted ship; arrow keys / PageUp / PageDown orbit and zoom; BACKSPACE clears a half-composed order; P pauses; F1 shows pulse / fleet rank / coefficients.
    Negative coefficients work too: hold S and the ghost points the other way.

📝 REPORT BACK

The usual — plus one thing I want your honest gamer opinion on: how does composing-then-committing feel? Too slow, too fast, satisfying, confusing? (The coefficient growth rate and the 0.5 snapping are feel-knobs I can change in one line.)

Next packages after your confirmation: the content loader + real ship meshes, then bridge (the Navigator's mouse console — your partner joins the game), then campaign with Mission 1 built to the minute-by-minute script. The cathedral has its first heartbeat and its first hands on the controls. 🚀❤️
