...
class RobotData:
    ...
    understanding_dir: str = ""  # Brief #A — baked-PNG folder (path string only; engine stays math-blind)
...
class CorridorData:
    ...
    ledger: ColorLedger
    understanding_dir: str = ""  # Brief #A — baked-PNG folder (path string only)
    ...
    for tok in toks[1:]:
        ...
        if keyword == "NAME":
            ...
            if not re.match(r'^[A-Za-z0-9_]+$', tok_id):
                raise ParseError(
                    f"{fname}:{ln}: VULNERABLE_TO id must be [A-Za-z0-9_]+, got {tok_id!r}"
                )
