...
- `style.css?v=13` → `style.css?v=15` bumped on all HTML pages (Python, UTF-8 safe) so the new menu CSS is fetched fresh.
- BUGFIX (v=15): the level-3 flyout originally opened to the RIGHT (`left:100%`) and ran off the right edge of the viewport (the nav is right-aligned), causing a horizontal scrollbar that shifted the whole page. Fixed by opening the flyout to the LEFT (`right:100%; left:auto`), moving the hover bridge to the left side, and pointing the level-2 caret left.
